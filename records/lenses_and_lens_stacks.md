...
Lenses are what things we are trying to keep consistent.
Sets of tiles and labels and their associated data can make up lenses. We've also described these to consultants as being a focus on a slice of a model.
...
#### Platform
Platforms are the shapes on a detail level that indicate a conceptual space, and are not "vignettes". Platforms are that which is not: background imagery for aesthic feel, vignettes to represent individual concepts, labels. Platforms may also be referred to by the illustration team as "doughnuts" where they are platforms which together make up a circular shape.


## Lenses in Adobe Illustrator & Semantic Tools

The concept of a lens on the Shared Meaning Platform must translate to how maps are created in and exported from Adobe Illustrator.
...
A set of detail levels which share the same geography and also have consistency of platforms remaining in the same location (XY coordinates) should be designed to create the illusion of zoom.
...
AJ feels the definition that a lens is a complete set of detail levels is more helpful.
...
Sets of lenses and detail levels are expected to share an coordinates (sit on same geography), which is not the case for the Artboard in Adobe Illustrator. The mere sharing of an artboard in Adobe Illustrator does not meet the threshold for the definition of a shared "geography".

### Splitting Across Adobe Illustrator Files
In theory, the detail levels that make up a lens should be contained in the same Adobe Illustrator file, but multiple lenses that make up a map should be able to be split across multiple Adobe Illustrator files.
...
A set common map IRI in the Semantic Tree Tools panel, ensures multiple maps that belong to the same client project and should appear on the same map IRI can be indicated across multiple Adobe Illustrator files.
...
The full range for zoom levels for a map is (at present) 0 - 7, but this is clamped to 0 - 6 by default.
...
Zoom levels have increasing visual fidelity of elements the higher the number goes. As such the geography where all elements sit on also has increased fidelity tied to resolution.
